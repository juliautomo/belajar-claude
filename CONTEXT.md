...
_Last updated: September 3, 2026 (checkpoint 231)_
...
## SHIPPED TO PRODUCTION (Checkpoint 223, August 22, 2026): Meta Pixel tracking for the first paid ads campaign, plus password toggle + SEO metadata cherry-picked from `dev` — both repos

**Status: shipped to `main` on both `belajar-claude` and `belajar-claude-backend` — live on `belajarclaude.id` and Railway production.**

**Meta Ads campaign infrastructure.** Julia is running a first low-budget test campaign (Rp20K/day, targeting the All Access Rp149K promo) and needed conversion tracking wired up before launch. Built the Meta Ads Manager setup with her live (dataset "BelajarClaude Website", ID `1488423606341237`; skipped Conversions API/Stape for this first pass — plain browser Pixel is enough for a Rp20K/day test) and implemented the resulting Pixel code in the codebase:
- New `meta-pixel.js` (site-wide, included on all 31 pages same pattern as `posthog-config.js`): fires `PageView`, but gated behind `if (location.hostname === 'belajarclaude.id')` — this site is a single Worker deployment served at both `belajarclaude.id` (prod) and `dev-belajar-claude.belajarclaude-id.workers.dev` (dev), same files, so without this gate every dev/preview pageview and test purchase would land in the same dataset as real ad traffic.
- `payment-success.html` fires `Purchase` with the *real* order amount, not a guess — `belajar-claude-backend/index.js`'s Duitku `returnUrl` now appends `&amount=${amount}&order=${orderId}` (backend already had both in scope), and the success page reads them off the query string. Deduped per order via `localStorage` so a page refresh doesn't double-count. Flagged to Julia as an approximation, not a guarantee: this page loads on redirect from Duitku's checkout, which for async methods (bank transfer VA) can happen before the transfer is actually confirmed by `/webhook/duitku` — fine for a first small-budget test; recommended moving the event server-side (Conversions API, gated on `resultCode === '00'`) if paid spend scales up.
- Verified genuinely live, not just deployed: fetched `meta-pixel.js` directly off production to confirm the gate shipped correctly, then (once Julia reported Meta's dashboard showing no activity) had her open DevTools Network tab — all three pixel requests (`fbevents.js`, the `1488423606341237?v=...` init call, and the `tr/?id=...&ev=...` beacon) returned `200`, proving the client-side tracking genuinely works; Meta's own Events Manager UI was just lagging/not bound to the test session, unrelated to our code.
- Confirmed the full chain is live end-to-end: fetched the Railway deploy log for backend's latest production deployment and found `💳 Duitku mode: PRODUCTION (real money) — host: api-prod.duitku.com`, confirming `DUITKU_ENV=production` is set — which matters because that's what makes `FRONTEND_URL` (and therefore `returnUrl`) resolve to `belajarclaude.id` rather than the dev hostname; without it the Purchase event could never fire for real traffic since the pixel gate only allows the prod hostname.
- Also advised against turning on Meta's "automatic advanced matching" (auto-scans page forms for email/phone, sends hashed versions to Meta) for now — the privacy policy doesn't yet disclose Meta Pixel usage at all (unlike PostHog, which got a disclosure row in checkpoint-222-era work); flagged as a follow-up, not done this checkpoint.

**Password show/hide toggle + SEO metadata cherry-picked from `dev` to `main`, Analisis Data course and Tiffany's admin-email addition deliberately left on `dev`.** Julia asked to sync specific pieces of `dev`'s ahead-of-`main` work without bringing over the in-progress "Analisis Data & Laporan" course (still being built jointly with Tiffany — checkpoints 221/222 on `dev`) or Tiffany's `ADMIN_EMAILS` addition. `dev` and `main` had diverged commit-hash-wise for some already-shared content (e.g. `dev`'s `038aa5b`/`bc2e791` duplicate content already on `main` via different hashes from an earlier merge), so rather than a blanket `git merge dev`, diffed the specific `dev` commits path-scoped to just the wanted files and hand-applied them: password toggle (`login.html`, `login-modal.js`, `reset-password.html` — eye icon show/hide, styled to match each surface's existing input box) and SEO meta tags (description, canonical, Open Graph/Twitter Card — `index.html`, `all-access.html`, `prompt-gratis.html`, `strategi-marketing.html`, `claude-sehari-hari.html`; skipped `analisis-data.html` since that page doesn't exist on `main` yet). Verified every touched file byte-for-byte against `dev`'s actual committed version afterward (only the expected `meta-pixel.js`-related and Analisis-Data-exclusion differences remained — nothing silently missed).

**Caught and fixed a real (pre-existing) bug during a post-push code check**: `reset-password.html`'s `:root` never defined `--ink-3`, `--surface-2`, or `--accent-glow` — present in `dev`'s own source commit (`923c2af`), not introduced by the cherry-pick — so the toggle icon's color, hover background, and focus ring would have silently failed to render there (invalid `var()` at computed-value time just gets dropped by the browser, not an error). Ported the same values `login.html` already had into `reset-password.html`'s `:root`. Also ran `node --check` on all touched JS/inline `<script>` blocks and an HTML tag-balance check across all 7 touched HTML files — clean.

**Infra finding, worth remembering for future sessions**: this session's Windows-mounted `belajar-claude`/`belajar-claude-backend` folders have a FUSE-level quirk where `rm`/unlink on a file fails with `Operation not permitted` (including git's own internal lock/temp-object cleanup, which fires on nearly every git command), but a same-directory `mv`/rename of the same file succeeds — so stray `.git/index.lock` (and similar) files can be cleared by renaming them aside rather than deleting, without needing Julia to intervene each time. Direct `git push` to both repos worked fine this session (no repeat of the checkpoint-218 git-proxy block) — commits went straight to `main` rather than through a PR. One real scare from this quirk: a `git merge` whose internal working-tree write failed silently *dropped* a few already-on-`dev`/already-in-transit content changes (the `comingSoon`/`dev_preview` fix in 3 files, a PostHog privacy-policy disclosure row) without any conflict marker or error — caught only by diffing every affected file's content hash against `origin/main` directly rather than trusting `git status`/`git diff` output, and manually restored. Lesson: after any merge on this mount, verify by content hash, not by git's own reported status.

**Commits this checkpoint** (`belajar-claude`, direct to `main`): `447d461` (Meta Pixel, on `feature/meta-pixel-tracking`), `a76048a` (merge `origin/main` into that branch, fast-forwarded to `main`), `e845eb3` (password toggle + SEO), `84f827e` (reset-password.html CSS var fix). (`belajar-claude-backend`, direct to `main`): `a3d3c302` (Duitku `returnUrl` amount/order params, built via `commit-tree` after `git merge` hit repeated lock contention on this mount). (Plus this checkpoint's own `CONTEXT.md` commit.)

## SHIPPED TO PRODUCTION (Checkpoint 224, August 22, 2026): Meta domain verification fix + InitiateCheckout/AddPaymentInfo funnel events

**Status: shipped to `main` on `belajar-claude`, live on `belajarclaude.id`.**

**Root-caused why Events Manager showed "No activity was received" / "No websites found"** despite checkpoint 223's confirmed-successful client-side pixel fires. Not a code problem: `belajarclaude.id` was unverified in Meta Business Settings → Domains, a separate requirement from installing the pixel itself. Added the meta tag Meta issued for verification (`<meta name="facebook-domain-verification" content="zvh14ksy3uyamhzbwg6123bugxy6fm" />`) to `index.html`'s `<head>` (static tag, no hostname gate needed — it's inert unless someone runs domain verification against that exact host, and only `main`/`belajarclaude.id` has it; confirmed by fetching both `belajarclaude.id` and the dev workers.dev hostname directly and diffing — dev's `index.html` doesn't have it, since dev is served from a different branch entirely, not just a JS-gated copy of the same file as originally assumed). Julia verified the domain in Business Settings; Events Manager subsequently confirmed 25 real events received from the Website source the same day.

**Chased down and ruled out a false alarm along the way**: Julia's own normal (non-incognito) Chrome browsing showed a Pixel-Helper-style extension reporting "No pixels found" on pages she'd visited before the pixel existed, while a hard refresh or fresh incognito window always showed it firing correctly. Diagnosed as ordinary browser HTTP caching of pre-pixel page versions in her personal profile, not a real gap — any actual ad-driven visitor arrives with an empty cache, same as incognito, so this never affected real traffic.

**Split checkout tracking into two standard Meta events instead of one**, on `all-access.html`, so the drop-off between "opened checkout" and "actually submitted payment info" is visible, and both stay eligible for ad-set optimization targeting in Ads Manager (custom event names lose that):
- `InitiateCheckout` now fires in `buyCourse()`, when the payment modal opens — the point Meta's own docs define as the actual start of checkout. Previously fired nothing at this step.
- The event checkpoint 223 called `InitiateCheckout` (fires in `submitPayment()` once a real Duitku payment reference exists, right before the buyer reaches the widget) is renamed to `AddPaymentInfo`, which is what it actually represents.
- Confirmed no duplication risk: every "Mulai Belajar Sekarang" link across the site (`index.html`, `produktivitas.html`, `mulai-claude.html`, `prompt-gratis.html`, `content-marketing.html`, `claude-sehari-hari.html`, `strategi-marketing.html`) is a plain `<a href="all-access.html">`, not a separate checkout trigger — PageView is the only signal until someone actually clicks "Beli All Access" on `all-access.html` itself. `submitPayment()`'s button disables on click, and a genuine retry after an error creates a new payment reference, correctly firing as a new event rather than a dupe of the first attempt.

**Commits this checkpoint** (`belajar-claude`, direct to `main`, all built via `commit-tree` plumbing after `git add`/`git commit` repeatedly hit the same FUSE lock/unlink issue documented in checkpoint 223 — one attempt silently produced an empty commit, caught immediately by diffing the commit's tree against the working file before pushing, and rebuilt correctly on top of the right parent): `50be44d` (domain verification meta tag), `3cff9f6` (InitiateCheckout added to `submitPayment()`), `9835626` (split into `InitiateCheckout`/`buyCourse()` + `AddPaymentInfo`/`submitPayment()`). (Plus this checkpoint's own `CONTEXT.md` commit.)

**Update, same day — full funnel confirmed Active end-to-end.** Test Events / dataset Overview stayed empty for a while even after real, confirmed-successful client-side fires (verified via DevTools Network tab showing genuine 200 responses from Facebook's servers for `InitiateCheckout`, same pattern already proven for `PageView` in checkpoint 223) — same Meta-dashboard-lag pattern as the domain-verification saga, not a code problem. Confirmed by directly re-checking the deployed Cloudflare Worker's `modified_on` timestamp (matched the latest push) to rule out stale-code as an explanation before concluding it was purely a display lag. Events Manager eventually caught up: all four events now show **Active** — `PageView` (44), `InitiateCheckout` (3), `AddPaymentInfo` (3), `Purchase` (2, used by 2 ad sets). The handful of test-fired events (via direct URL visits, no real payment) have no `fbclid`/click ID attached, so they won't be attributed to the campaign's actual ad-level performance reporting once it's running for real — only inflate this raw diagnostic count, which doesn't feed into billing or optimization. Also caught during ad setup: the ad's Website URL was defaulting to the bare homepage rather than `all-access.html` — fixed before publishing, so ad clicks land directly on the offer instead of one hop away. Campaign duration set to **3 days (through August 25, 2026)** rather than the original 7-day plan — even tighter for exiting Meta's learning phase on Purchase at Rp20K/day, already-known tradeoff from the original plan doc, not a new concern.

---

## SHIPPED TO PRODUCTION (Checkpoint 225, August 24, 2026): Post-launch security/legal hardening pass — CORS lockdown, rate limiting, input validation, security headers, SEO coverage, admin-email cleanup — both repos

**Status: shipped to `main` on both `belajar-claude` and `belajar-claude-backend` — live on `belajarclaude.id` and Railway production.** Everything below went `dev` first, was verified live there, then promoted to `main` — re-established as the standing default this checkpoint after a workflow correction (see below).

**Workflow correction, worth remembering.** Julia stopped mid-session: *"remember, we need to update to dev first then main, check our workflow md please, adjust the dev accordingly, i said dont touch tiffany, but workflow stays the same, except the meta, cos we dont want to duplicate."* Everything earlier this session (the CORS fix below) had gone straight to `main`. Re-read `TEAM-WORKFLOW.md` and corrected course: `dev` is the default target for anything that isn't Meta-Pixel-specific, `main` only on an explicit ask (or automatically for Meta Pixel work, the one standing exception, since duplicating that onto `dev` would double-count events), and Tiffany's `dev`-only in-progress content (the Analisis Data course) is never touched. Applied consistently for the rest of the session.

**Backend: CORS lockdown + rate limiting.** `index.js`'s CORS was `origin: '*'` — any website's JS could call `/signup` and `/create-payment`. Scoped to the two real frontend hostnames (`belajarclaude.id`, the `workers.dev` dev preview). Added `express-rate-limit` (20 req/15min/IP) to both endpoints — not `/webhook/duitku`, which is server-to-server from Duitku and already protected by signature verification. Caught one edge case before it shipped: the literal string `"null"` (what browsers send as `Origin` for `file://` pages — `backend-config.js` explicitly routes local file testing to the dev backend) wasn't matched by the `!origin` check, which would have broken that workflow; added `origin === 'null'` to the allow-list check. Verified via a real click-through: Julia tested the dev checkout flow and confirmed it still reached the Duitku sandbox widget.

**Backend: helmet security headers.** Added `helmet()` — strips the `X-Powered-By: Express` header, adds `X-Content-Type-Options`, HSTS, `Referrer-Policy`. One real gotcha caught in local testing before it ever touched `dev`: helmet's default `Cross-Origin-Resource-Policy: same-origin` would have blocked `belajarclaude.id`'s own cross-origin `fetch()` calls to this Railway domain (verified directly — spun up a local Express+helmet instance, confirmed the header value, confirmed overriding it to `crossOriginResourcePolicy: { policy: 'cross-origin' }` fixes it without reintroducing the `origin: '*'` problem, since CORS above still does the real access control).

**Backend: input validation on `/signup` and `/create-payment`.** Both endpoints previously only checked truthiness — objects, arrays, numbers, whitespace-only strings all passed silently through to ConvertKit, Supabase, Sheets, and Duitku's invoice API. Added `validateName`/`validateEmail` helpers: real type check, trim, length caps (name 100 / email 254), proper email regex. Found and fixed a real parsing bug along the way, not just theoretical hardening: `customerName` is joined into Duitku's `additionalParam` as `` `${name}||${email}` `` and split back apart on the webhook side — a name containing `|` (e.g. `"X||someone-else@example.com"`) would shift what the webhook parsed as the email, meaning a paid course's access/set-password email could land somewhere the buyer never typed into the email field. Rejecting `|` in both fields closes that off. Separately confirmed (re-reading the handler, not assuming) that payment amount was never client-tamperable in the first place — `/create-payment` never reads a client-supplied amount; the price is always looked up server-side via `getEffectivePrice()`. Verified the validators directly with a dozen hand-written cases (legit Indonesian emails incl. `+`-addressing and `.co.id` domains, the `|`-smuggling attempt, type confusion, oversized input) before shipping — all behaved correctly.

**Frontend: `robots.txt` + `sitemap.xml` added — site had neither.** `sitemap.xml` covers the 11 real public/indexable pages. `robots.txt` disallows admin/dashboard/account pages, the login-gated `*-content.html` course-material pages, the community feed, and five retired meta-refresh redirect stubs. Caught and self-corrected a real mistake here: the first pass wrongly listed `kursus-karyawan.html`, `kursus-mahasiswa.html`, `kursus-ukm.html`, `paket.html`, and `paket-content-creator.html` in the sitemap as if they were real pages — they're actually meta-refresh redirect stubs to `all-access.html` (per-course purchases are retired, same pattern as `kebijakan-pengembalian.html`). Removed them from `sitemap.xml`, added them to `robots.txt`'s disallow list instead. Separate finding, not fixed (out of reach from here): `belajarclaude.id/robots.txt` doesn't actually serve this file — Cloudflare has a zone-level "Managed robots.txt"/AI Crawl Control feature on for this domain that fully overrides whatever the origin serves. `sitemap.xml` itself is unaffected and serves correctly; Google Search Console submission works regardless of the `Sitemap:` line being absent from the (Cloudflare-overridden) `robots.txt`. Flagged to Julia — turning it off, if she wants the custom rules live, is a Cloudflare dashboard setting, not something reachable from this session.

**Frontend: SEO meta tags on the 6 remaining real public pages** (`mulai-claude.html`, `produktivitas.html`, `content-marketing.html`, `faq.html`, `kebijakan-privasi.html`, `syarat-ketentuan.html`) — title/description/canonical/OG/Twitter tags, same pattern already live on the other 5 pages since checkpoint 222/223. Every real public page is now covered.

**Self-caught workflow bug worth documenting in detail, since it's a real repeat-risk on this mount.** Partway through the SEO work, `dev`'s checkout had drifted to reflect `main`'s file content — a side effect of using `git symbolic-ref HEAD refs/heads/dev` (pure ref manipulation, used repeatedly this session to work around the FUSE lock issue) without ever actually running a working-tree checkout, so the Windows-mounted files being read/edited were still `main`'s. First attempt at the 6-page SEO edit would have leaked `main`-only Meta Pixel content onto `dev`: the `meta-pixel.js` `<script>` include on all 6 pages, plus the full Meta Pixel disclosure paragraph and third-party-provider table row on `kebijakan-privasi.html`. Caught before pushing (`dev` and `origin/dev` were never out of sync publicly) by diffing `dev`'s pre-edit blobs against `main`'s for each file — confirmed the *only* pre-existing difference was the single `meta-pixel.js` line, confirming this was a fresh mistake, not an already-shipped state. Rebuilt all 6 files by extracting `dev`'s actual blob content directly via `git cat-file -p` (bypassing the compromised working-tree checkout entirely) and re-applying only the SEO block via Python, verified each diff was exactly 13 clean insertion lines with nothing else touched, then force-updated the local `dev` ref to the corrected commit before pushing. Fetched the live `dev` preview afterward to confirm `kebijakan-privasi.html` showed no Meta Pixel content. Lesson for future sessions: `git symbolic-ref HEAD` changes what a *new* commit's parent will be, not what the working-tree files on disk currently contain — always extract content via `git cat-file -p <tree-ish>:<path>` when building a commit for a branch other than whatever the Windows checkout currently reflects, never trust a Read-tool file read to match the target branch.

**Promoting to `main`, file-aware rather than a branch merge.** `dev` and `main` have diverged content (Meta Pixel is `main`-only sitewide: the `meta-pixel.js` include, `all-access.html`'s pixel events, `kebijakan-privasi.html`'s disclosure, `index.html`'s domain-verification tag — plus Tiffany's in-progress Analisis Data course exists only on `dev`), so nothing this checkpoint was merged or fast-forwarded — each change was applied against `main`'s actual current file content and diffed before committing, same discipline as checkpoint 223's cherry-pick. Backend had no such divergence (confirmed by diffing `main:index.js` against `dev:index.js` before promoting — the entire diff was exactly the helmet + validation change, nothing else), so those blobs were reused directly. Frontend's 6 SEO pages were rebuilt from `main`'s own blobs (not `dev`'s) with only the SEO block added, verified diff-clean against `main` for all 6 files before pushing; `robots.txt`/`sitemap.xml` are new files with no divergence risk. Verified live on `belajarclaude.id` after deploy: `kebijakan-privasi.html` shows both the new SEO tags *and* the Meta Pixel disclosure intact; `sitemap.xml` serves correctly (after a short Cloudflare propagation delay — first fetch came back empty, resolved on retry). Backend Railway production deploy confirmed via logs: clean boot, `DUITKU_ENV=production` intact, `helmet`/validation active.

**Database cleanup, no repo commits.** Julia asked to remove `tffnyy26@gmail.com` from the database entirely. Enumerated every RLS policy referencing it first (`pg_policies` query) rather than guessing — 19 policies across 9 tables (`community_comments`, `community_posts` ×2, `course_pricing` ×2, `course_pricing_history` ×2, `course_resources` ×2, `course_visibility`, `module_documents` ×2, `module_ppts` ×3, `module_videos` ×3, `social_links`), all following the same 3-email admin-array pattern. Applied via `ALTER POLICY` migration replacing the array in all 19 with just `julia.utomo@gmail.com`/`tiffany.utomo@gmail.com`, verified 0 remaining references afterward via the same query. Brings the database back in sync with `admin.html`'s client-side `ADMIN_EMAILS` array, which never had this email in the first place. Also confirmed no refund-policy content exists anywhere real on the live site per Julia's question — `syarat-ketentuan.html` has no refund/guarantee section, the only "refund" text on the site is unrelated course-lesson copy in `produktivitas-content.html`/`mulai-claude-content.html`, and `kebijakan-pengembalian.html` is a harmless orphaned redirect stub with zero incoming links (fine to leave as-is or delete, Julia's call, not acted on either way).

**Also investigated, no code issue found.** Julia flagged Meta Events Manager's Dataset Overview showing a PageView count drop (44 → 14) and a missing "today" data point. Clarified this is a fundamentally different view than Ads Manager's Results column — Datasets Overview shows all raw events from every source mixed together (uncapped by campaign attribution), and Meta's own post-ingestion bot/quality filtering can revise counts downward after the fact; not a sign of real data loss or a code regression.

**Commits this checkpoint.** `belajar-claude-backend`, `dev`: `1c81f2b` (CORS scoping + rate limiting), `7b0a4be` (null-origin fix), `6e61a20` (helmet), `ed1e066` (input validation). `belajar-claude-backend`, `main`: `2b22fe4` (CORS+rate-limit+null-origin, combined), `44ce978` (helmet+validation, combined). `belajar-claude`, `dev`: `202abda` (robots.txt/sitemap.xml), `cce8507` (SEO tags on 6 pages + sitemap/robots redirect-stub fix — rebuilt once after the working-tree mixup described above, never pushed in its tainted form). `belajar-claude`, `main`: `7fc7331` (robots.txt/sitemap.xml/SEO tags, combined, built against `main`'s own content). Database: 1 migration (`remove_tffnyy26_admin_email`), no repo commit. (Plus this checkpoint's own `CONTEXT.md` commit.)


**Update, same day — redirect-stub cleanup, Purchase-event trace, and next ad set up in Ads Manager.**

**`kebijakan-pengembalian.html` deleted from `main`.** Follow-up to the "no refund content, harmless orphan" finding above — Julia said to delete it. Confirmed (again, directly against `main`'s content, not assumed) zero incoming links anywhere on `main` before removing it, and dropped its now-unneeded `robots.txt` disallow entry in the same commit. Verified live: `belajarclaude.id/kebijakan-pengembalian.html` now 404s. **Left in place on `dev`** — `analisis-data.html` (Tiffany's in-progress, dev-only course page) has a real footer link to it ("Kebijakan Pengembalian Dana"), so deleting the file there would break that link. Flagged to Julia; not acted on further without her direction on whether to also drop the link from Tiffany's page.

**Traced the 2 "Purchase" events Julia spotted in Events Manager (Dataset Overview screenshot) rather than assuming they were real sales.** First confirmed dev genuinely cannot be the source: fetched `meta-pixel.js` from both hostnames directly — production serves the file with the `location.hostname === 'belajarclaude.id'` gate intact, the dev preview returns nothing at all (the file isn't even deployed there, not just gated). Then cross-referenced backend logs against `enrollments`: 4 Duitku invoices were created on Aug 22 (production backend logs, deployment `6a925ffb`) — 2 from `belajarclaude.id@gmail.com` (the site's own contact address, almost certainly Julia's own test) and 2 from `topoci.pekanbaru@gmail.com`/`topocipekanbaru@gmail.com` (same person, dot-variant retry 9 minutes apart — reads as a real ad-driven visitor, not internal testing). None of the 4 produced a webhook confirmation or an `enrollments` row — the `enrollments` table's most recent paid `all-access` row is still Aug 18, older than any of this. Conclusion: the Purchase pixel fires on landing at `payment-success.html`, which for VA/bank-transfer checkout happens as soon as Duitku issues a virtual account number — before any money actually moves. So "Purchase: 2" here means 2 people reached that step, not 2 completed sales; likely one of Julia's own tests and one real visitor who started checkout and didn't finish paying. Reinforces the approximation caveat already flagged in checkpoint 223 about this event.

**New ad in progress in Ads Manager** ("BC - Membership Rp149K - Test 01" → "Business - Indonesia 18-45" → "New Sales ad") — helped with ad copy (3 Primary text variants, 2 Headline options, 1 Description, CTA recommendation to switch off the default "Learn More" toward "Shop Now"/"Get Access" since the destination is a direct paid page). Advised against Meta's multi-text-variant testing feature for this specific ad given the Rp20K/day budget — splitting that little spend across variants means none gets enough impressions to show a real difference; recommended just shipping the single strongest Primary text option instead. Clarified the Tracking-panel URL-parameters field is optional (Pixel conversion tracking already works without it; only useful for segmenting this traffic inside PostHog, which auto-captures UTM params) and separately caught that the ad's required **Website URL field was empty** — pointed her to `https://belajarclaude.id/all-access.html` specifically, the same destination-URL mistake checkpoint 224 already found and fixed once on the first ad.

**Commits this update:** `belajar-claude`, `main`: `5504f5d` (delete `kebijakan-pengembalian.html` + its `robots.txt` entry). Ads Manager changes are Julia's own, made directly in Meta's UI — no repo commits. (Plus this update's own `CONTEXT.md` commit.)
