...
_Last updated: July 11, 2026 (checkpoint 3)_
...
- **RLS fix + PPT feature (July 11, 2026)**: the "Dokumen Praktik" upload was failing with "new row violates row-level security policy" because the `course-documents` storage bucket was never actually created (the RLS policies in `admin-content-setup.sql` section 5 had nothing to attach to). `sql/module-ppts-and-fix-uploads.sql` fixes this — it creates `course-documents` and the new `course-ppts` bucket via SQL (`insert into storage.buckets ...`, no manual dashboard step needed), sets up `module_ppts` + its RLS policies, and re-applies the `course-documents` storage policies. **This script still needs to be run in the Supabase SQL editor** — not yet confirmed executed as of this checkpoint.
