...
_Last updated: July 17, 2026 (checkpoint 12)_
...
- **Important discovery made during this reorg**: the actual module PPT files (`K2-M01` through `K2-M08`, 8 files) that tasks earlier in this session record as "generated" were **never actually committed to git** — `git log --all` shows zero history for them anywhere in the repo. They must have existed only as local scratch/output files from an earlier part of this session (before the context-compaction point) and were wiped by the `git clean -fd` pass described above (stale-mount fix), since they were untracked. Only `K2-M1-preview.pptx` (a preview file, not a full module deck) survived. **This needs to be regenerated from scratch** — tracked as an open task. Course *content* on the live site is unaffected; this is purely about the underlying source PPT files.
...
**Open tasks tracked (not yet done)**:
1. Regenerate the Mulai Claude Module 2 PDF panduan + PPT to reflect the R-K-T-F fix.
2. Regenerate all 8 Produktivitas Kantor module PPTs (M01–M08) — lost, never actually committed (see above).
