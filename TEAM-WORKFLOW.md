...
- **Checkout and sign-up are also shared and real, for now.** The "Beli All Access" button and the sign-up form both talk to our one real payment/email backend — so clicking through a purchase or creating an account on the dev link creates a **real payment attempt and a real customer account**, exactly like on the live site. Don't test the buy flow or sign-up flow on `dev` expecting it to be a safe sandbox — it isn't yet. (Update, same day this was written: the code has since been restructured so this *can* be split — see the "Payments: dev vs. production" section below — but the actual separation isn't finished, so this caveat still applies until it is.)
...
**What's been set up so far**: the backend code now supports both environments via one setting (`DUITKU_ENV` on Railway — defaults to sandbox if unset, so nothing changes automatically). The frontend's backend URL has also been consolidated into one file (`backend-config.js`) instead of being copy-pasted across four pages, so a future dev/prod split only requires editing that one file.

**What's still needed to make dev genuinely safe for checkout testing**: a second backend service on Railway, pointed at the `dev` branch, configured with sandbox Duitku credentials and `DUITKU_ENV` left unset — separate from the real production service. That's a manual Railway dashboard setup, not something Claude can provision directly. Until that exists, checkout/sign-up stays in the "always real, be careful" category above regardless of which link you're using.
